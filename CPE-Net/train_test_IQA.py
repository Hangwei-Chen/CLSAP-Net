...
def main(config):
    ...
    cp_srcc_all = np.zeros(config.train_test_num, dtype=float)
    cp_krcc_all = np.zeros(config.train_test_num, dtype=float)
    cp_plcc_all = np.zeros(config.train_test_num, dtype=float)
    cp_rmse_all = np.zeros(config.train_test_num, dtype=float)
    ...
    for p in range(config.train_test_num):
        ...
        cp_srcc_all[p], cp_krcc_all[p],cp_plcc_all[p], cp_rmse_all[p]= solver.train()



    srcc_CP_med = np.mean(cp_srcc_all)
    krcc_CP_med = np.mean(cp_krcc_all)
    plcc_CP_med = np.mean(cp_plcc_all)
    rmse_CP_med = np.mean(cp_rmse_all)



    print('Testing CP_SRCC %4.4f,\tCP_krcc %4.4f,\tCP_PLCC %4.4f,\tCP_RMSE %4.4f' % (srcc_CP_med, krcc_CP_med,plcc_CP_med, rmse_CP_med))
