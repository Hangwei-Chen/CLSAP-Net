...
def main(config):
    ...
    ov_srcc_all = np.zeros(config.train_test_num, dtype=float)
    ov_krcc_all = np.zeros(config.train_test_num, dtype=float)
    ov_plcc_all = np.zeros(config.train_test_num, dtype=float)
    ov_rmse_all = np.zeros(config.train_test_num, dtype=float)
    ...
    for p in range(config.train_test_num):
        ...
        # Randomly group: 5 subsets, and use 5-fold leave-one-out cross-validation
        group_label = []
        ...
        # select 80% groups for training and the rest for testing
        for j in range(150):
            e = type_index[j]
            for i in range(8):
        ...
        ov_srcc_all[p], ov_krcc_all[p],ov_plcc_all[p], ov_rmse_all[p] = solver.train()


    srcc_OV_med = np.mean(ov_srcc_all)
    krcc_OV_med = np.mean(ov_krcc_all)
    plcc_OV_med = np.mean(ov_plcc_all)
    rmse_OV_med = np.mean(ov_rmse_all)

    print('Testing OV_SRCC %4.4f,\tOV_krcc %4.4f,\tOV_PLCC %4.4f,\tOV_RMSE %4.4f' % (srcc_OV_med, krcc_OV_med, plcc_OV_med, rmse_OV_med))
